...
## Текущий статус
- **Задача:** Отключение FPN-эффекта с апом на карты на мобильных устройствах
- **Этап:** BUILD — реализация завершена, требуется проверка на устройствах
...
- [x] Добавлена проверка на мобильное устройство через `window.matchMedia('(pointer: coarse)')` в компоненте `Card.jsx`
- [x] Отключена логика `hide-peek` анимации на мобильных устройствах (при `clickCount >= 3` на мобильных просто сбрасывается счётчик без запуска анимации)
- [x] Проверка линтера: ошибок нет
- [x] Проверка сборки: `npm run build` успешно (991ms)

## Следующие шаги
- Проверка на мобильном устройстве (эффект hide-peek не должен срабатывать при трёх тапах)
- Проверка на десктопе (эффект должен работать как раньше)
- После проверки — переход в `/reflect`
