...
## Статус
- В работе (UI): отключён normal-режим (UI скрыт) и CASCADE теперь всегда по умолчанию:
  - `useBalatroInfernoController`: добавлен флаг `NORMAL_MODE_ENABLED=false`, `readStoredGameMode()` теперь игнорирует `repoker.gameMode=normal` и возвращает `cascade`.
  - `BalatroInferno`: верхняя панель упрощена — убрана надпись `MODE: ...` и сегмент выбора режима; `PAYTABLE` перенесён в левый верхний угол; DEV-кнопка сделана текстовой `/dev`.
  - Для будущего возврата: UI переключения режимов оставлен под флагом `MODE_SWITCHER_UI_ENABLED=false`.

Дальше: прогнать гейты (`npm run lint` / `npm run build`) и быстро руками проверить (Paytable, /dev).

Гейты (после отключения normal/UI): `npm run lint` — ✅, `npm test` — ✅ (24/24), `npm run build` — ✅.

Апдейт (UI-шапка): ряд `PAYTABLE` + `/dev` перенесён под плашки `CHIPS/ANTE` и выровнен по колонкам; к `PAYTABLE` добавлена пиксельная money-иконка с лёгкой “клеткой” (`PixelMoneyIcon` + `.pixel-grid-overlay`). Гейты: `npm run lint` — ✅.

- В работе (UI mobile/узкие): поправлена шапка `CHIPS / MAX WIN / ANTE` в `BalatroInferno`:
  - уменьшены минимальные размеры `clamp()` для надписи **MAX WIN** и значения **150.000x** на mobile (base), чтобы реально сжималось на <640px.
  - перераспределена ширина grid на mobile: центральная колонка уже, боковые (CHIPS/ANTE) шире.
  - суммы в CHIPS/ANTE переведены на `truncate` + уменьшен min-size на mobile.
  - для mobile включён более “агрессивный” `formatMoneyAdaptive` (раньше уходит в compact), чтобы реже резать длинные суммы.

Гейты: `npm run lint` — ✅, `npm run build` — ✅ (прогон вне sandbox из‑за `EPERM` на чтении `node_modules` в sandbox).

Дальше: `/reflect`.

Апдейт (hotfix): исправлен белый экран — `chipsDisplay` использовал `isMobileViewport` до объявления (TDZ/`ReferenceError`). `isMobileViewport` (`useState` + `matchMedia` effect) перенесён выше, перед вычислением `chipsDisplay`. `npm run build` — ✅.

Апдейт (responsive MAX WIN): “MAX WIN 150.000x” теперь масштабируется **от ширины центральной колонки**, а не от viewport — добавлены container queries (`cqw`) с fallback на текущие `vw`-классы.

Апдейт (упрощение по фидбэку): на **mobile (< 640px)** “MAX WIN 150.000x” в шапке **отключён** (скрыт), шапка становится 2‑колоночной: **CHIPS / ANTE**. На `sm+` (>=640px) остаётся 3‑колоночной с MAX WIN по центру. Гейты: `npm run lint` — ✅, `npm run build` — ✅.

Архив: `memory-bank/archive/archive-2025-12-23-mobile-header-maxwin-hide.md`
