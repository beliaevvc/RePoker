...
- [x] Хотфикс: устранён белый экран (рантайм `ReferenceError` из-за использования `runMaxWinCinematic` до объявления в вычислении `mobilePerfAnimating`). Переставлено вычисление ниже объявления `runMaxWinCinematic`. Тех‑проверка: `npm run build` проходит.
