...
## Этапы (7–8 этапов, каждый с подэтапами)
...
**Цель**: сделать качественную адаптацию под мобильные устройства (от маленьких экранов до планшетов), без потери визуала и без изменения игровой логики.

Подэтапы:
- Определить “целевые” брейкпоинты и проверить:
  - iPhone SE / small (≈320px)
  - стандартные телефоны (360–430px)
  - планшеты (768–1024px)
- Привести layout к mobile-first:
  - нормализовать размеры карт, отступы, зоны нажатия
  - проверить, что элементы управления (ставка/PLAY) удобно нажимаются (touch targets)
  - исключить горизонтальный скролл/обрезания
- Оптимизировать тяжёлые эффекты на мобилках:
  - проверить производительность (FPS/лаг) на слабых устройствах
  - при необходимости ввести “умные” деградации (например уменьшить blur/кол-во слоёв) **без изменения логики**
- Обновить стили/компоненты так, чтобы они были предсказуемо responsive (Tailwind utilities + локальный CSS).
- Дополнить smoke‑чеклист мобильными пунктами (см. ниже).

Гейт:
- `npm run build` проходит
- smoke‑чеклист + мобильный smoke‑чеклист проходит (ручная проверка в DevTools device emulation)

Мобильный smoke‑чеклист (добавка):
- Ничего не вылезает за экран, нет горизонтального скролла.
- Все кнопки легко нажимаются, нет “промахов”.
- Текст читаем, цифры не обрезаются.
- Анимации не “убивают” FPS (допустимо упростить эффекты только визуально).
